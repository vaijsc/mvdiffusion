...
def main(args):
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet, mv_teacher_lora):
                ...
                if mode == "mv":
                    ...
                    set_unet_self_attn_cross_view_processor(unet_model=unet, num_views=1)
                    set_unet_self_attn_cross_view_processor(unet_model=mv_teacher_lora, num_views=1)
                    ##########################
                    ...
                    # Get predicted original samples from unet
                    pred_original_samples = predict_original(unet, noise_scheduler, input_noise, prompt_embeds, None).to(
                        dtype=weight_dtype
                    )   # camera = None
                    ...
                    with torch.no_grad():
                        accelerator.unwrap_model(mv_teacher_lora).disable_adapters()
                        teacher_pred_cond = mv_teacher_lora(noisy_samples, timesteps, prompt_embeds, None).sample
                        teacher_pred_uncond = mv_teacher_lora(noisy_samples, timesteps, prompt_null_embeds, None).sample

                        accelerator.unwrap_model(mv_teacher_lora).enable_adapters()
                        lora_pred_cond = mv_teacher_lora(noisy_samples, timesteps, prompt_embeds, None).sample
                    ...
                    # Compute output for updating the LoRA teacher
                    encoder_hidden_states = prompt_null_embeds if random.random() < 0.1 else prompt_embeds
                    lora_pred = mv_teacher_lora(noisy_samples, timesteps, encoder_hidden_states, None).sample
