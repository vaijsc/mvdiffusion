...
def parse_args(input_args=None):
    ...
    parser.add_argument("--guidance_scale_max", type=float, default=7.5, help="The classifier-free guidance scale.")
    parser.add_argument("--guidance_scale_min", type=float, default=7.5, help="The classifier-free guidance scale.")
...
def main(args):
    ...
    # Init student
    unet = MVDreamUNet2DConditionModel.from_pretrained(
        args.pretrained_model_name_or_path, subfolder="unet", revision=args.revision
    )
    ...
    if args.use_ema:
        ema_unet = MVDreamUNet2DConditionModel.from_pretrained(
            args.pretrained_model_name_or_path, subfolder="unet", revision=args.revision
        )
        ema_unet = EMAModel(ema_unet.parameters(), model_cls=MVDreamUNet2DConditionModel, model_config=ema_unet.config)
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def load_model_hook(models, input_dir):
            if args.use_ema:
                load_model = EMAModel.from_pretrained(os.path.join(input_dir, "unet_ema"), MVDreamUNet2DConditionModel)
            ...
            for i in range(len(models)):
                ...
                if isinstance(model, type(accelerator.unwrap_model(unet))):
                    load_model = MVDreamUNet2DConditionModel.from_pretrained(input_dir, subfolder="unet")
    ...
    mv_teacher_lora, sd_teacher_lora = accelerator.prepare(mv_teacher_lora, sd_teacher_lora)
    lr_scheduler_mv_lora, lr_scheduler_sd_lora = accelerator.prepare(lr_scheduler_mv_lora, lr_scheduler_sd_lora)
    optimizer_mv_lora, optimizer_sd_lora = accelerator.prepare(optimizer_mv_lora, optimizer_sd_lora)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet, mv_teacher_lora, sd_teacher_lora):
                ...
                if mode == "mv":
                    ...
                    #random camera
                    with torch.no_grad():
                        elevation_cam = random.randint(0, 30)
                        azimuth_start_cam = random.randint(0, 360)
                        cam_dist = 1.0
                    ...
                    input_shape = (bsz * 4, 4, args.resolution // 8, args.resolution // 8)
                    ...
                    original_samples = predict_original(unet, noise_scheduler, input_noise, prompt_embeds, camera_embeds).to(
                        dtype=weight_dtype
                    )
                    pred_original_samples = torch.nn.functional.interpolate(original_samples, scale_factor=0.5, mode="bilinear")
                    ...
                    with torch.no_grad():
                        ...
                        guidance_scale = (args.guidance_scale_max - args.guidance_scale_min) * random.random() + args.guidance_scale_min
                    ...
                    if accelerator.sync_gradients:
                        ...
                        for name, param in unet.named_parameters():
                            # if "attention" in name:
                            params_2d_to_clip.append(param)
                        accelerator.clip_grad_norm_(params_2d_to_clip, args.max_grad_norm)
                    ...
                    # Get predicted original samples from unet
                    pred_original_samples = predict_original(unet, noise_scheduler, input_noise, prompt_embeds, camera_embeds).to(
                        dtype=weight_dtype
                    )
                    # VSD loss
                            ...
                            for tracker in accelerator.trackers:
                                for prompt in args.validation_prompts:
                                    for i, image in enumerate(images[prompt]):
                                        ...
                                        os.makedirs(os.path.join(visualize_dir, f"step_{global_step}"), exist_ok=True)
                                        torchvision.utils.save_image(image.unsqueeze(0)/255, os.path.join(visualize_dir, f"step_{global_step}", f"{prompt}_{i}.png"))
